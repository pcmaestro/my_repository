...
def borrar_coche(id_registro):
    print("LLego a operaciones_bd.borrar_coche()")    
...
def obtener_coche_por_id(id_registro):    
    ...
    mydb.commit()
    mydb.disconnect()
    objeto_coche = clases.Coche(marca = coche_obtenido[1], modelo = coche_obtenido[2], color = coche_obtenido[3], motor = coche_obtenido[4], precio = coche_obtenido[5])
...
def guardar_cambios_coche(cambios_ptes_guardar):    
    ...
    mydb = conectar()
    cursor = mydb.cursor()
    val = (cambios_ptes_guardar.marca, cambios_ptes_guardar.modelo, cambios_ptes_guardar.color, cambios_ptes_guardar.motor, cambios_ptes_guardar.precio, cambios_ptes_guardar.id )
    cursor.execute(sql, val)
    mydb.commit()
    mydb.disconnect()
